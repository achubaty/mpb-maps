...
rasterOptions(
  maxmem = 1e+12,
  tmpdir = checkPath(file.path(dirname(tempdir()), "scratch", "MPB"), create = TRUE)
)
...
if (grepl("achubaty", ._USER_.)) {
  if (._OS_. == "Darwin") {
    ...
    } else if (grepl("HST-NRX-CTX-09", Sys.info()[["nodename"]])) {
      maps.dir <- "W:/HealthyLandscapes/Data"
      work.dir <- "~/GitHub/mpb-maps"
    } else {
      stop("data and maps dirs need to be specified for your system")
    }
...
if (isTRUE(._MAPS_USERAW_.) && (._OS_. == "Windows") &&
  ...
  fc <- st_read(dsn = f, layer = "STAND_SUSC_INDEX_MPB")
  
  ## save to shapefiles
  output.dir <- file.path(maps.dir, "MPB", "ab_mpb_ssi") %>% checkPath(create = TRUE)
  
  fout <- file.path(output.dir, "ab_mpb_ssi.shp")
  write_sf(fc, dsn = fout)
}
```
...
if (isTRUE(._MAPS_USERAW_.) && (._OS_. == "Windows") &&
  ...
  ## save to shapefiles
  output.dir <- file.path(maps.dir, "MPB", "ab_mpb_2018") %>% checkPath(create = TRUE)
