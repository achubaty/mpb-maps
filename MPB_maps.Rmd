...
if (!require(amc) || packageVersion("amc") < "0.1.0.9000") {
...
if (isTRUE(._MAPS_USERAW_.)) {
  ...
  shapefile(west.boreal, file.path(rdata.path, "west_boreal.shp"), overwrite = TRUE)
  fout <- file.path(rdata.path, "west_boreal.tif")
  gdal_rasterize(file.path(rdata.path, "west_boreal.shp"), dst_filename = fout,
                 a = "OBJECTID", l = "west_boreal", tr = c(._RES_MAPS_., ._RES_MAPS_.))
  west.boreal.raster <- raster(fout)
  rm(fout)
...
if (isTRUE(._MAPS_USERAW_.) && (._OS_. == "Windows") &&
  ...
  lapply(ab_pnts, function(x) {
    ...
    shapefile(x, filename = f, overwrite = TRUE)
  })
  lapply(ab_poly, function(x) {
    ...
    shapefile(x, filename = f, overwrite = TRUE) ## throws warnings about field widths
  })
}
```
...
**NOTE:** Until we resolve how to handle point/polygon overlap, we are only using the AB points data.
...
if (isTRUE(._MAPS_USERAW_.)) {
  ...
  sfInit(cpus = ._NUM_CPUS_., parallel = TRUE) 
    ...
    sfLibrary(rgeos)
    sfLibrary(sp)
    sfExport("crs.boreal", "maps.dir", "output.dir", "rdata.path", "west.boreal.raster")
    ...
    ab.pnts.boreal.raster.stack <- sfClusterApplyLB(ab.pnts.dir.shp, function(shp) {
      ...
      if (is.null(pnts.boreal@data$NUM_TREES)) {
      ...
      # TO DO: can this be done using gdal_rasterize; how to do the 'fun = sum' piece?
...
data1999 <- list(
  paste0(url, "1999/shape/prov_ibm/PROV_IBM_point_point.dbf"),
  paste0(url, "1999/shape/prov_ibm/PROV_IBM_point_point.prj"),
  paste0(url, "1999/shape/prov_ibm/PROV_IBM_point_point.shp"),
  paste0(url, "1999/shape/prov_ibm/PROV_IBM_point_point.shx"),
  paste0(url, "1999/shape/prov_ibm/PROV_IBM_poly_poly.dbf"),
  paste0(url, "1999/shape/prov_ibm/PROV_IBM_poly_poly.prj"),
  paste0(url, "1999/shape/prov_ibm/PROV_IBM_poly_poly.shp"),
  paste0(url, "1999/shape/prov_ibm/PROV_IBM_poly_poly.shx")
)
data2000 <- list(
  paste0(url, "2000/provincial_data/shape/2000%20IBM/prv_ibm.dbf"),
  paste0(url, "2000/provincial_data/shape/2000%20IBM/prv_ibm.shp"),
  paste0(url, "2000/provincial_data/shape/2000%20IBM/prv_ibm.shx"),
  paste0(url, "2000/provincial_data/shape/2000%20IBM/prv_spot.dbf"),
  paste0(url, "2000/provincial_data/shape/2000%20IBM/prv_spot.shp"),
  paste0(url, "2000/provincial_data/shape/2000%20IBM/prv_spot.shx")
)
data_zip <- list(
  ...
  paste0(url, "2016/AOS_2016_Shapefiles_and_TSA_Spreadsheet_Jan25.zip")
)
...
if (isTRUE(._MAPS_USERAW_.)) {
  ...
  lapply(bc_pnts, function(f) {
    ...
    shapefile(tmp[ids, ], filename = file.path(output.dir, paste0("ibm_spot_", year)),
              overwrite = TRUE)
  })
  ...
  lapply(bc_poly, function(f) {
    ...
    shapefile(tmp[ids, ], filename = file.path(output.dir, paste0("ibm_poly_", year)),
              overwrite = TRUE)
  })
  ...
  checkProjections <- function(type) {
    ...
    allPrjEqual <- suppressWarnings(sapply(pnts.prj, readLines)) %>%
      unique() %>%
      length() %>%
      `==`(1)
    if (isTRUE(allPrjEqual)) {
      lapply(missing.prj, function(f) file.copy(pnts.prj[1], f))
    } else {
...
if (isTRUE(._MAPS_USERAW_.)) {
  ...
  sfInit(cpus = ._NUM_CPUS_., parallel = TRUE) 
    ...
    sfLibrary(rgeos)
    sfLibrary(sp)
    sfExport("._RES_MAPS_.", "crs.boreal", "maps.dir", "output.dir", "rdata.path", "west.boreal.raster")
...
if (isTRUE(._MAPS_MKRSTR_.)) {
  ...
  files <- dir(rdata.path, pattern = ".tif", full.names = TRUE) %>% 
  ...
  sfInit(cpus = ._NUM_CPUS_., parallel = TRUE) 
    ...
    bcab.boreal.raster.stack <- sfClusterApplyLB(sort(unique(yrs)), function(y) {
      ...
      stack() %>% 
      writeRaster(filename = file.path(rdata.path, "mpb_bcab_boreal_1997-2011.grd"),
                  overwrite = TRUE)
  sfStop()    
  
  writeRaster(bcab.boreal.raster.stack,
              filename = file.path(rdata.path, "mpb_bcab_boreal_1997-2011.tif"),
  ...
  writeRaster(bcab.boreal.raster.brick,
              filename = file.path(rdata.path, "mpb_bcab_boreal_1997-2001_brick.tif"),
              overwrite = TRUE) ## TO DO: add options to compress raster depending on size
