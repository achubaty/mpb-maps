...
if (isTRUE(._MAPS_USERAW_.)) {
  ...
  sfInit(cpus = ._NUM_CPUS_., parallel = TRUE) 
    ...
    # save these new map objects for later use
    saveObjects(c("ab.pnts", "ab.pnts.boreal", "ab.pnts.boreal.raster.stack"), rdata.path)
  sfStop()
  
  ## cleanup workspace
  rm(ab.pnts.dir.shp, ab.pnts.files, ab.pnts.boreal, ab.pnts.boreal.raster.stack, west.boreal.raster)
  .cleanup()
}
```
...
if (isTRUE(._MAPS_USERAW_.)) {
  ...
  sfInit(cpus = ._NUM_CPUS_., parallel = TRUE) 
    ...
    sfLibrary(rgeos)
    sfLibrary(sp)
    sfExport("crs.boreal", "getOGR", "maps.dir", "west.boreal.raster")
    ...
    bc.poly.boreal.raster.stack <- sfClusterApplyLB(bc.poly.dir.shp, function(shp) {
      ...
      out <- rasterize(poly.boreal, west.boreal.raster, getCover = TRUE) / 100 * 1125 * (._RES_MAPS_. / 100) ^ 2
      year <- strsplit(shp, "_")[[1]][3]
      writeRaster(out, filename = file.path(rdata.path, paste0("MPB_BC_poly_", year, ".grd")),
                  overwrite = TRUE)
      writeRaster(out, filename = file.path(rdata.path, paste0("MPB_BC_poly_", year, ".tif")),
                  overwrite = TRUE)
      return(out)
    }) %>% 
...
if (isTRUE(._MAPS_MKRSTR_.)) {
  ...
  for (i in 1:nlayers(bc.poly.boreal.raster.stack)) {
    if (any(names(bc.poly.boreal.raster.stack)[i] == names(ab.pnts.boreal.raster.stack))) {
      ...
      if (any(is.finite(cellStats(bc.poly.boreal.raster.stack[[i]], "range")))) {
        all[[i]] <- merge(ab.pnts.boreal.raster.stack[[wh.pnts[j]]],
                          bc.poly.boreal.raster.stack[[wh.poly[j]]])
      } else {
        all[[i]] <- ab.pnts.boreal.raster.stack[[wh.pnts[j]]]
      }
    } else {
      all[[i]] <- bc.poly.boreal.raster.stack[[i]]
    }
  }
  ...
  bcab.boreal.raster.stack <- stack(all) %>%
    setNames(substr(names(bc.poly.boreal.raster.stack), 2, 5))
  ...
  bcab.boreal.raster.stack <- writeRaster(bcab.boreal.raster.stack,
                                          filename = file.path(rdata.path, "bcab.boreal.raster.stack.grd"),
                                          overwrite = TRUE)
    
  saveObjects(c("bcab.boreal.raster.brick", "bcab.boreal.raster.stack"), rdata.path)
  
  ## cleanup workspace
  rm(all, bcab.boreal.raster.brick, bcab.boreal.raster.stack,
     ab.pnts.boreal.raster.stack, bc.poly.boreal.raster.stack,
     wh.pnts, wh.poly)
  .cleanup()
}
```
