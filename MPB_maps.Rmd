...
use.cache = TRUE          # cache knitr results
...
```{r import.maps-points, echo=FALSE, cache=use.cache}
if (read.in.raw.maps) {
...
```{r import.maps-polygons, echo=FALSE, cache=use.cache}
if (read.in.raw.maps) {
  if (!exists("WORKSPACE")) source("workspace.maps.R")
...
```{r import.maps-boreal, echo=FALSE, cache=use.cache}
if (read.in.raw.maps) {
  if (!exists("WORKSPACE")) source("workspace.maps.R")
  ...
  ### CANADIAN MAPS
  # fill in code to read in can adm maps from gadm.org
  # e.g. from my MPB stuff (AB & SK)
  # canada <- readShapeSpatial("CAN_adm/CAN_adm1.shp", proj4string=CRS("+proj=longlat +datum=WGS84"))
  # canada2 <- spTransform(canada, CRS("+proj=lcc +lon_0=110w +lat_1=49n +lat_2=60n"))
  ...
  west.boreal.union.buff = gBuffer(west.boreal, width=1e-5) # corrects for non-adjacent boundaries
  #boreal.west = intersect(west.boreal.union.buff, boreal.can)
  boreal.west = gIntersect(west.boreal.union.buff, boreal.can, byid=TRUE)
  rm(west.boreal.union.buff)
...
```{r crs.boreal, echo=FALSE, cache=use.cache, results="markup"}
if (!exists("WORKSPACE")) source("workspace.maps.R")
...
```{r reproject.maps-points, echo=FALSE, cache=use.cache}
if (reproj.raw.maps) {
  if (!exists("WORKSPACE")) source("workspace.maps.R")
...
```{r reproject.maps-polygons, echo=FALSE, cache=use.cache}
if (reproj.raw.maps) {
  if (!exists("WORKSPACE")) source("workspace.maps.R")
...
```{r rasterize.maps-boreal, echo=FALSE, cache=use.cache}
if (rasterize.maps) {
  if (!exists("WORKSPACE")) source("workspace.maps.R")
...
```{r rasterize.maps-points, echo=FALSE, cache=use.cache}
if (rasterize.maps) {
  if (!exists("WORKSPACE")) source("workspace.maps.R")
...
```{r rasterize.maps-polygons, echo=FALSE, cache=use.cache}
if (rasterize.maps) {
  if (!exists("WORKSPACE")) source("workspace.maps.R")
...
```{r combine.maps-all, echo=FALSE, cache=use.cache}
if (rasterize.maps) {
  if (!exists("WORKSPACE")) source("workspace.maps.R")
...
```{r plot.canada.boreal, fig.height=12, fig.width=16, echo=FALSE, cache=use.cache}
if (!exists("WORKSPACE")) source("workspace.maps.R")
...
```{r plot.western.boreal, fig.height=12, fig.width=16, echo=FALSE, cache=use.cache}
if (!exists("WORKSPACE")) source("workspace.maps.R")
...
```{r plot.mpb.western.boreal.time.series, fig.height=12, fig.width=12, echo=FALSE}
if (!exists("WORKSPACE")) source("workspace.maps.R")
...
rm(bcab.all.boreal.raster.stack, brk.ll, ts.kml, ts.rts west.boreal)
```
