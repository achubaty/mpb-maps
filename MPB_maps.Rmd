...
if (isTRUE(._MAPS_USERAW_.)) {
  ...
  # this next line is BROKEN; not sure what happened :(
  boreal.west <- intersect(boreal, west.boreal) %>%
    gBuffer(., byid = TRUE, width = 0)
  
  f <- normalizePath(file.path(rdata.path, "west_boreal.shp"))
  shapefile(west.boreal, f, overwrite = TRUE)
  fout <- normalizePath(file.path(rdata.path, "west_boreal.tif"))
  gdal_rasterize(src_datasource = f, dst_filename = fout,
                 a = "OBJECTID", l = "west_boreal",
                 tr = c(._RES_MAPS_., ._RES_MAPS_.)) # TODO: use fasterize (requires sf)
  ...
  #             overwrite = TRUE) # TODO: use fasterize (requires sf)
  # )
...
if (isTRUE(._MAPS_USERAW_.)) {
  ...
  sfInit(cpus = ._NUM_CPUS_., parallel = TRUE) 
    ...
    ab.pnts.boreal.raster.stack <- sfClusterApplyLB(ab.pnts.dir.shp, function(shp) {
      ...
      # TODO: use fasterize (requires sf)
