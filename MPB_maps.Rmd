...
if (isTRUE(._MAPS_USERAW_.)) {
  ...
  west.boreal <- st_transform(canada1.west, crs.boreal)
  west2.boreal <- st_transform(canada2.west, crs.boreal)
  canada1.boreal <- st_transform(CAN_adm1, crs.boreal)
  canada2.boreal <- st_transform(CAN_adm2, crs.boreal)
...
if (isTRUE(._MAPS_USERAW_.)) {
  ...
  sfInit(cpus = ._NUM_CPUS_., parallel = TRUE) 
    ...
    ab.pnts.boreal.raster.stack <- sfClusterApplyLB(ab.pnts.dir.shp, function(shp) {
      ...
      pnts.boreal <- st_transform(pnts, crs.boreal)
  ...
  sfInit(cpus = ._NUM_CPUS_., parallel = TRUE) 
    ...
    bc.poly.boreal.raster.stack <- sfClusterApplyLB(bc.poly.dir.shp, function(shp) {
      poly.boreal <- getOGR(shp, output.dir) %>% 
        ...
        spTransform(., CRS(crs.boreal$proj4string)) %>%
