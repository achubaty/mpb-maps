...
## additional options
num.cpus <- parallel::detectCores() / 2 ## max cpus to use
...
if (isTRUE(read.in.raw.maps)) {
  ...
  boreal.west <- foo()    ## INCOMPLETE
  canada1.boreal <- foo() ## INCOMPLETE
  canada2.boreal <- foo() ## INCOMPLETE
  west.boreal <- foo()    ## INCOMPLETE
  west2.boreal <- foo()   ## INCOMPLETE
...
#  names(bcab.all.boreal.raster.stack) <- substr(names(bcab.poly.boreal.raster.stack), 2, 5)
  ...
  #                                             filename = file.path(rdata.path, "mpb_brick.grd"),
  #                                             overwrite = TRUE)
  writeRaster(bcab.all.boreal.raster.stack,
              filename = file.path(rdata.path, "bcab.all.boreal.raster.stack.grd"),
              overwrite = TRUE)
    
  saveObjects(c(#"bcab.all.boreal.raster.brick",
                "bcab.all.boreal.raster.stack"), rdata.path)
  
  ## cleanup workspace
  rm(all, #bcab.all.boreal.raster.brick,
     bcab.pnts.boreal.raster.stack, bcab.poly.boreal.raster.stack,
     bcab.all.boreal.raster.stack, wh.pnts, wh.poly)
  .cleanup()
}
```
