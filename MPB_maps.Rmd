...
## additional options
._NUM_CPUS_. <- min(parallel::detectCores() / 2, 19) ## max cpus to use
...
if (isTRUE(._MAPS_USERAW_.)) {
  if (isTRUE((._OS_. == "Windows") && (NROW(subset(ogrDrivers(), grepl("GDB", name))) > 0))) {
    ...
    lapply(years, function(x) {
      ...
      #poly <- st_read(dsn = gisDBconn, layer = paste0("MPB_AB_", x, "_poly")) ## many years missing poly data
      ab_pnts[[i]] <<- pnts
      #ab_poly[[i]] <<- poly
      i <<- i + 1
    })
...
if (isTRUE(._MAPS_USERAW_.)) {
  ...
  writeRaster(ab.pnts.boreal.raster.stack, file.path(rdata.path, "MPB_AB_pnts_2001-2019.tif"))
