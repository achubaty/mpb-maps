...
rasterOptions(
  maxmem = 1e+12,
  tmpdir = checkPath("/tmp/scratch/MPB", create = TRUE)
)
...
if (._USER_. == "achubaty") {
  if (._OS_. == "Darwin") {
    ...
    if (grepl("W-VIC-A105254", Sys.info()[["nodename"]])) {
      maps.dir <- "G:/data"
    } else {
...
._CRS_MAPS_. <- structure(list(
  epsg = NA_integer_,
  proj4string = paste("+proj=lcc +lat_1=49 +lat_2=77 +lat_0=0 +lon_0=-95",
                      "+x_0=0 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs")
), class = "crs")         ## map projection to use (matches kNN data)
```
...
if (isTRUE(._MAPS_USERAW_.)) {
  ...
  for (s in 0:3) {
    assign(paste0("CAN_adm", s),
           as(raster::getData('GADM', path = adm.path, country = "Canada", level = s), "sf"),
           envir = .GlobalEnv)
...
if (isTRUE(._MAPS_USERAW_.)) {
  ## TODO: use prepInputs to get and load the boreal shapefile
  ...
  if (all(file.exists(files))) {
    ...
    boreal <- read_sf(f) %>% st_transform(._CRS_MAPS_.)
  ...
  west.boreal <- st_transform(canada1.west, ._CRS_MAPS_.)
  west2.boreal <- st_transform(canada2.west, ._CRS_MAPS_.)
  canada1.boreal <- st_transform(CAN_adm1, ._CRS_MAPS_.)
  canada2.boreal <- st_transform(CAN_adm2, ._CRS_MAPS_.)
...
if (isTRUE(._MAPS_USERAW_.)) {
  ...
  sfInit(cpus = ._NUM_CPUS_., parallel = TRUE) 
    ...
    sfLibrary(raster)
    sfLibrary(sf)
    sfExport("._CRS_MAPS_.", "maps.dir", "output.dir", "rdata.path", "west.boreal.raster")
    ...
    ab.pnts.boreal.raster.stack <- sfClusterApplyLB(ab.pnts.dir.shp, function(shp) {
      ...
      pnts.boreal <- st_transform(pnts, ._CRS_MAPS_.)
...
if (isTRUE(._MAPS_USERAW_.)) {
  ...
  cl <- parallell::makeForkCluster(._NUM_CPUS_.)
  ...
  bc.poly.boreal.raster.stack <- parallel::parLapplyLB(cl, bc.poly.dir.shp, function(shp) {
  ...
  ._CRS_MAPS_. = ._CRS_MAPS_.,
  ._RES_MAPS_. = ._RES_MAPS_.,
  maps.dir = maps.dir,
  output.dir = output.dir,
  rdata.path = rdata.path, 
  west.boreal.raster = west.boreal.raster) %>% 
