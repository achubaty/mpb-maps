...
._RES_MAPS_. <- 2000      ## map resolution, in metres
```
...
if (isTRUE(._MAPS_REPROJ_.)) {
  ...
  sfInit(cpus = ._NUM_CPUS_., parallel = TRUE)
    ...
    bcab.pnts.boreal <- sfClusterApplyLB(1:length(wh.ab), function(x) {
      ...
      setNames(names(bc.poly.boreal)[wh.bc])
...
if (isTRUE(._MAPS_MKRSTR_.)) {
  ...
  ### we use 1125 trees per 1ha, per Whitehead & Russo (2005), Cooke & Carroll (unpublished)
  change.res <- function(x, y = west.boreal.raster, field = 1125, fun = "last", ...) {
  ...
  bcab.poly.boreal.raster.stack <- stack(bcab.poly.boreal.raster.unstack) %>%
    setNames(names(bc.poly.boreal.raster.stack))
