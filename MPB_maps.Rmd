...
if ((._OS_. == "Windows") && (NROW(subset(ogrDrivers(), grepl("GDB", name)))) > 0) {
  ...
  ## save to shapefiles
  output.dir <- file.path(maps.dir, "MPB", "ab_mpb_AMC")
  if (!dir.exists(output.dir)) dir.create(output.dir)
  lapply(ab_pnts, function(x) {
    year <- names(x) ## IS THIS CORRECT??
    f <- filepath(output.dir, paste0("ab_mpb_", year, "spot.shp"))
    shapefile(x, filename = f, overwrite = TRUE)
  })
  lapply(ab_poly, function(x) {
    year <- names(x) ## IS THIS CORRECT??
    f <- filepath(output.dir, paste0("ab_mpb_", year, "poly.shp"))
    shapefile(x, filename = f, overwrite = TRUE)
  })
}
```
...
if (isTRUE(._MAPS_USERAW_.)) {
  ...
  ab.pnts.files <- dir(path = file.path(maps.dir, "MPB", "ab_mpb_AMC"), pattern = "spot")
...
```{r download-bc-mpb}
bc.data.dir <- file.path(maps.dir, "MPB", "bc_mpb")
...
for (i in 3:length(years)) {
  tryCatch(
    dl.data(data_zip[[i - 2]], dest = file.path(bc.data.dir, years[i]), unzip = TRUE),
    error = function(e) warning(e)
  )
}
```
...
output.dir <- file.path(maps.dir, "MPB", "province_BC_AMC")
if (!dir.exists(output.dir)) dir.create(output.dir)
...
lapply(bc_pnts, function(f) {
  ...
  year.id <- which(strsplit(f, "/")[[1]] == "bc_mpb") + 1
...
if (isTRUE(._MAPS_USERAW_.)) {
  ...
  bc.poly.files <- dir(path = file.path(maps.dir, "MPB", "province_BC_AMC"), pattern = "poly")
